...
MainframeWindow::MainframeWindow(HWND hWnd):
    ...
    drawingTexture(0),
    grabbedImageBuffer(NULL)
{
    memset(&header, 0, sizeof(ToupTekCameraManager::FrameHeader));
...
MainframeWindow::~MainframeWindow()
{
    ...
    memset(&exposureTimeRangeDTO, 0, sizeof(ToupTekCameraManager::RangeDTO));
    memset(&exposureGainRangeDTO, 0, sizeof(ToupTekCameraManager::RangeDTO));
    ...
    memset(&header, 0, sizeof(ToupTekCameraManager::FrameHeader));
    if (grabbedImageBuffer) {
        free(grabbedImageBuffer);
        grabbedImageBuffer = NULL;
    }
}
...
void MainframeWindow::drawRightChild()
{
    ...
    if (ImGui::BeginChild("##viewport", ImVec2(0, ImGui::GetContentRegionAvail().y), flags)) {
        ...
        if (manager.grabImageData(header, grabbedImageBuffer)) {
            glBindTexture(GL_TEXTURE_2D, drawingTexture);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, header.width, header.height, 0,
                GL_RED,
                header.bpp <= 8 ? GL_UNSIGNED_BYTE : GL_UNSIGNED_SHORT,
                grabbedImageBuffer);
        }

        if (drawingTexture && header.width && header.height) {
            ImVec2 space = ImGui::GetContentRegionAvail();

            float arImage  = float(header.width) / header.height;
            ...
            ImGui::SetCursorPos(ImVec2((space.x - imgSize.x)/2, (space.y - imgSize.y) / 2));
            ImGui::Image((ImTextureID)(intptr_t)drawingTexture, imgSize);
        }
